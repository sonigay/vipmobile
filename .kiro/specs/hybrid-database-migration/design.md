...
**마이그레이션 우선순위:**
1. **Phase 1 (검토 대상)**: 총 37개 시트 (내일 사용자와 함께 최종 선정)
   - 직영점 모드 (14개 시트)
   - 정책 모드 (10개 시트)
   - 고객 모드 (7개 시트)
   - 온세일 모드 (3개 시트)
   - 온세일관리 모드 (온세일 모드와 동일 시트 사용)
   - 예산 모드 (3개 시트)
   - 일반정책 모드 (정책 모드와 동일 시트 사용, 권한 필터링만 다름)
...
**Phase 1 마이그레이션 대상 (검토 중 - 총 44개 시트):**
...
**온세일 모드 & 온세일관리 모드 (3개 시트):**
- 개통정보 (동적 시트 - 온세일링크관리에서 지정된 시트 ID 사용)
- 온세일링크관리 (링크 설정 및 개통양식 시트 ID 관리)
- 정책게시판 (CRUD 빈번)
...
- 예산_대상월관리
- 예산_사용자시트관리
- 기본구두 (동적 시트 - 사용자별 시트 ID 사용)
- 기본구두생성목록
- 예산데이터 (동적 시트 - 사용자별 시트 ID 사용)
- 폰클개통데이터 (다른 모드와 공유)
- 기본데이터 (동적 시트 - 사용자별 시트 ID 사용)
- 기본데이터생성목록

**일반정책 모드:**
- 정책 모드와 동일한 시트 사용 (권한 필터링만 다름)
- 별도 시트 없음

**구글 시트 유지 (마이그레이션 제외):**
- **일반모드권한관리** - 권한 설정 시트 (수동 편집 필요, 구글 시트 유지)

**참고:** 
- 위 시트 목록은 검토 대상이며, 내일 사용자와 함께 실제 마이그레이션 대상을 최종 선정할 예정입니다.
- 온세일 모드와 온세일관리 모드는 동일한 시트를 사용합니다 (권한만 다름).
- 일반정책 모드는 정책 모드와 동일한 시트를 사용합니다 (권한 필터링만 다름).
- 예산 모드는 폰클개통데이터 시트를 다른 모드와 공유하여 사용합니다.
- 예산 모드의 일부 시트는 동적으로 생성되는 사용자별 시트입니다 (기본구두, 예산데이터, 기본데이터).
