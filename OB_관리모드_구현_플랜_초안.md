...
- 최근 업데이트: 2025-11-12 (각 섹션별 그래프/차트 추가)
- 관련 모드: OB 관리모드
...
- ✅ 각 섹션별 그래프/차트 추가: 최종정산(항목별 금액 비교), 맞춤제안(정책별 금액 비교), 재약정(수수료/오퍼 비교 및 등록직원별 금액), 인건비/비용(인건비/비용 비교 및 시트/수기 입력 비교)
- ⚙️ 남은 TODO (고도화):
...
- [x] 각 섹션별 그래프/차트 추가: 섹션 펼침 시 하단에 표시 (recharts 사용)
- [x] 각 금액 카드에 건수 표기 추가
...
- [x] 각 섹션별 접기/펼치기 토글 기능 추가 (최종정산, 맞춤제안, 재약정, 인건비/비용) - 기본값 접힘 상태
...
- [x] 그래프/차트는 주요 수치보다 하단에 배치(참고용), 필요 시 토글로 숨김 처리 (각 섹션 펼침 시 표시)
...
- [x] 월별 필터 선택 시 정책①~③, 건수 구간별 지급 금액을 요약 카드로 표시 (정책① 기본, 정책② 테마 업셀, 정책③ 인건비 지원, 건수 구간별 지급 카드)
...
- [x] 정책별 금액 비교 그래프: 섹션 펼침 시 하단에 표시 (정책① 기본, 정책② 테마 업셀, 정책③ 인건비 지원, 건수 구간별 지급)
...
- [x] 오퍼 금액 산출 로직:
  ...
  - [x] 시트에 이미 -금액으로 입력되므로 그대로 사용 (*-1 처리 제거)
  - [x] 상품권/입금 구분 없이 전체 오퍼금액으로 통합 처리
  - [x] 재약정 오퍼 지급 카드에 안내 멘트 추가: "오퍼금액은 폰클에서 입력시 홈 추가정책 란에 입력해주세요"
...
- [x] 수수료/오퍼 비교 그래프: 섹션 펼침 시 하단에 표시
- [x] 등록직원별 금액 그래프: 상위 10개 등록직원의 수수료/오퍼 스택 차트 (섹션 펼침 시 하단에 표시)
...
- [x] 인건비/비용 비교 그래프: 섹션 펼침 시 하단에 표시
- [x] 시트/수기 입력 비교 그래프: 인건비 시트/수기, 비용 시트/수기 비교 차트 (섹션 펼침 시 하단에 표시)
...
| 2025-11-12 | 각 섹션별 그래프/차트 추가: 최종정산(항목별 금액 비교), 맞춤제안(정책별 금액 비교), 재약정(수수료/오퍼 비교 및 등록직원별 금액), 인건비/비용(인건비/비용 비교 및 시트/수기 입력 비교) | assistant |
...
| 2025-11-12 | 재약정 오퍼금액: *-1 처리 제거 (시트에 이미 -금액으로 입력), 안내 멘트 추가 | assistant |
| 2025-11-12 | 각 섹션별 접기/펼치기 토글 기능 추가: 최종정산, 맞춤제안, 재약정, 인건비/비용 섹션에 토글 버튼 추가 (기본값 접힘) | assistant |
| 2025-11-12 | 플랜 문서 정리: 실제 구현 상태 확인 후 체크박스 업데이트 (정책①~③, 건수 구간별 지급 카드 표시 완료 확인) | assistant |
...
- [x] `시트이름(맞춤제안)` 30/31번 컬럼(영업팀/코드)을 유치자별 상세 내역 테이블에 표시 (부분 구현)
- [ ] 영업팀/코드 조합별 통계 제공 (UNIQUE 처리하여 팀/코드별 집계) - 향후 구현
- [x] 맞춤제안 월별 상세 패널(정책①~③, 건수 구간, 제외내역)을 카드/표 형태로 제공 (정책①~③, 건수 구간별 지급 카드, 제외 건수 표시)
