...
- 최근 업데이트: 2025-11-11 (대상점 관리 기능 추가)
- 관련 모드: OB 관리모드
- 신규 탭: `투게더계산기`(현재 화면), `OB 정산 확인`, `OB 정산 관리`
- 진행 현황: **1차 구현 완료 + 진행상황 저장 고도화 반영**
- 다음 액션:
  1. 남은 고도화 항목(팀/코드 통계, 재약정 상세 다운로드, 인건비 상세 등) 우선순위 확정
  2. 테스트 시나리오(12장) 최신 상태로 업데이트
  3. 추가 UX 개선 및 알림/이력 기능 여부 협의

-### 0.1 현재 진행 상황 요약 (2025-11-11 기준)
- ✅ `OB 정산 확인` 탭 기능 완성: 맞춤제안/재약정 계산, 수기 인건비·비용 CRUD, XLSX 다운로드, 요약 카드/테이블
- ✅ 월별 메타데이터/수기 입력/제외 인원/정산 진행상황을 구글 시트에 저장·복원 (링크 관리, `기타` 시리즈, `제외인원`, `정산진행상황`)
- ✅ 최종 정산 카드 워크플로우 개선: 단계별 Step UI + 카드 재배치, 진행도에 따라 강조/완화 및 안내 문구 표시, 완료 배너 상단 노출
- ✅ `OB 정산 관리` 탭: 월별 링크 CRUD, 제외 인원 CRUD, 진행상황 시트 저장 연동
- ⚙️ 남은 TODO (대표):
  - 맞춤제안 팀/코드 통계, 상세 패널/검색 필터, 재약정 다운로드 옵션 확장, 제외 인원 관리 고도화
  - 인건비/비용 추가 시각화, PDF/CSV 확장, 알림/이력/시뮬레이션 등 고도화 아이디어 검토
...
- [x] 맞춤제안/재약정 각각 유치자별/등록직원별 상세 금액 테이블 추가
...
- [ ] 시트 컬럼 30(영업팀), 31(코드) 값을 `UNIQUE` 처리하여 팀/코드 조합을 구분해 표시 (필터/요약 등 활용)
...
- [x] 오퍼 금액 산출 로직:
  ...
  - [x] 59번 컬럼(`동판-비고`)에서 "상품권" 혹은 "입금" 패턴을 찾아 금액 추출
  - [x] 74번 컬럼(`재약정-비고`)에서도 동일 패턴을 검색하여 금액 추출
  - [x] 상품권과 입금 항목을 구분하여 각각 합산(표시)하고, 총 오퍼 금액(마이너스) 계산
  - [x] 추출 시 숫자 파싱/오류 처리 로직 필요
...
- [x] **대상점 관리 기능 추가 (2025-11-11)**:
  ...
  - [x] 대상점 CRUD UI 구현 (연월/대상점명/사유/비고 입력)
  - [x] 대상점 시트(`대상점`) 자동 생성 및 헤더 구조 설정 (1행 빈 행, 2행 헤더)
  - [x] 재약정 필터링 로직 수정: OR 조건 적용
    - 제외인원(91인덱스 등록직원)에 없거나
    - 출고처(10인덱스)에 대상점 문자열이 포함된 경우만 포함
  - [x] 대상점 시트 헤더 컬럼 (2행): `연월 | 대상점명 | 사유 | ID | 비고 | 등록자 | 등록일시`
...
### 3.2.3 인건비
- [x] 월별 데이터는 `시트이름(후정산)` 시트에서 조회 (원시 데이터 제공 전까지 0원 기본값)
- [x] 추가/수기 입력 데이터는 `기타후정산` 시트에 저장 후 함께 집계
- [x] 인건비 관련 지표(월별 합계, 수기 입력 건수)를 UI에서 즉시 반영
...
- [x] 합산 로직: 시트 값(기본 0) + 수기 입력 금액을 합산하여 월별 인건비 합계에 반영
...
- [ ] **참고:** 후정산 원시 데이터 구조가 확정되면 시트 기반 자동 합산 로직 고도화 예정
...
- [x] 합산 로직: 시트 값(기본 0) + 수기 입력 금액을 합산하여 월별 비용 합계에 반영
...
- [ ] 구체적인 항목/산출식은 향후 시트 데이터 제공 시 확정
- [ ] 음수/양수 여부, VAT 포함 여부 등 표기 규칙 정의 (현행: 모든 수기 입력은 음수 처리)
- [ ] **참고:** 후정산 원본 데이터 연동 시 수기 입력과 병합하는 로직 추가 예정

- [x] **정산확인 최종 합산 로직**
  - [x] 월별 필터로 선택한 달에 대해 `맞춤제안`, `재약정`, `인건비(수기)`, `비용(수기)`을 합산
  - [x] 합산 결과를 최종 정산 금액으로 표시
  - [x] 각 항목별 금액과 최종 합계를 함께 보여주는 요약 카드 구성 (수기 입력/시트 값 구분 안내)
...
- [ ] 추가 입력 데이터는 `기타맞춤제안`, `기타재약정`, `기타후정산` 시트에 저장하도록 분리 관리
- [ ] **제외 대상 관리 UI**: 유치자마당ID, 유치자명을 입력해 제외 목록에 추가/삭제할 수 있는 필드 제공
- [ ] 제외 목록은 별도 시트(예: `기타맞춤제안` 내 서브 섹션 또는 신규 시트)에 저장하여 맞춤제안 계산 시 반영
- [ ] 입력 시 중복/형식 검증 및 확인 모달 제공
- [ ] 재약정 제외 인원 관리 UI를 동일 화면에서 제공(탭/아코디언로 구분), 맞춤제안과 별도 리스트 유지
- [ ] 제외 인원 리스트는 등록자/등록일시를 함께 저장해 추적 가능하도록 구성
...
2. 대상점 추가 버튼 클릭 → 모달에서 연월/대상점명/사유/비고 입력 후 저장 → 테이블에 신규 행 추가 확인
