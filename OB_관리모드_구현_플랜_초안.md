...
- 진행 현황: 초기 기획/설계 단계
- 다음 액션 (사용자 요구사항 수집 전 단계):
  1. 각 탭별 요구사항을 사용자에게서 상세히 파악
  2. 확정된 요구사항을 본 문서에 반영
  3. 구현 범위와 일정 재설정
...
- [ ] `정산관리` 탭에 재약정용 제외 인원 관리 UI 제공 (맞춤제안 제외 인원과 별도로 유지)
- [ ] 재약정 제외 목록은 별도 저장소(예: `기타재약정` 내 서브 시트 또는 신규 시트)에 기록
- [ ] 제외 대상은 `시트이름(재약정)` 91번 컬럼(`유치자마당ID`)과 비교하여 필터링
