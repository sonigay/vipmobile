...
### ✅ 테스트 3: 개통정보입력페이지 기본 동작 확인
**날짜**: 2026-01-18  
**결과**:
- 페이지 이동: ✅ 성공
- 요금제군 일치: ✅ 일치
- 초기 대리점추가지원금: ✅ 정상 표기
- 부가서비스 및 보험 적용시 금액 변경 섹션: ✅ 처음부터 정상 표기
- 부가서비스 로드 후 중복 계산: ✅ 해결됨 (부가서비스 로드 후 대리점추가지원금 표시)

### ✅ 테스트 4: 부가서비스 선택/삭제 테스트
**날짜**: 2026-01-18  
**결과**:
- 부가서비스 선택 시: ✅ 증가함 (정확히 40,000원 증가)
- 증가 금액 정확성: ✅ 정확함 (incentive 30,000원 + deduction 절댓값 10,000원 = 40,000원)
- 부가서비스 삭제 시: ✅ 감소함 (정확히 40,000원 감소)
- 감소 금액 정확성: ✅ 정확함 (40,000원 정확히 감소)

### ✅ 테스트 5: 가입유형 변경 후 부가서비스 제거 문제 해결 확인
**날짜**: 2026-01-18  
**테스트 항목**: 가입유형 변경 후 첫 번째 부가서비스 제거 시 금액 감소 확인
**결과**:
- 초기 상태 부가서비스 제거/추가: ✅ 정상 작동
- 가입유형 변경 (번호이동 → 기기변경) 후 첫 번째 부가서비스 제거: ✅ 금액 정상 감소
- 다시 가입유형 변경 (기기변경 → 번호이동) 후 첫 번째 부가서비스 제거: ✅ 금액 정상 감소
- 문제 해결 확인: ✅ 완료

**수정 내용**:
- `loadAvailableItems` 의존성에 `formData.openingType` 추가
- `initialSelectedItemsRef` 재설정 로직 개선
- 가입유형 변경 시 `selectedItems`가 재설정되도록 수정

### ✅ 테스트 6: 가입유형 변경 시 재계산 확인
**날짜**: 2026-01-18  
**테스트 항목**: 가입유형 변경 시 대리점추가지원금 재계산 및 부가서비스 선택 유지 확인
**결과**:
- 가입유형 변경 시 대리점추가지원금: ✅ 재계산됨 (가입유형별로 정상적으로 재계산)
- 부가서비스 선택 유지: ✅ 유지됨 (가입유형 변경 후에도 부가서비스 선택 상태 유지)

**확인 사항**:
- 가입유형 변경 시 대리점추가지원금이 누적되지 않고 새로운 가입유형에 맞는 금액으로 재계산됨
- 부가서비스 선택 상태가 가입유형 변경 후에도 유지됨

### ✅ 테스트 7: 약정유형별 표기 테스트
**날짜**: 2026-01-18  
**테스트 항목**: 일반약정과 선택약정에서 대리점추가지원금 표기 확인
**결과**:
- 일반약정 선택 시 표기: ✅ 차액만큼만 표기됨 (정상 작동)
- 선택약정 선택 시 표기: ✅ 전체 금액으로 표기됨 (정상 작동)
- 저장 시 값: ⏳ 확인 예정 (나중에 확인 예정)

**확인 사항**:
- 일반약정 선택 시 대리점추가지원금이 차액(대리점추가지원금 - 이통사지원금)으로 표기됨
- 선택약정 선택 시 대리점추가지원금이 전체 금액으로 표기됨
- 저장 시 실제 계산된 값이 저장되는지 확인 필요 (추후 테스트 예정)

### ✅ 테스트 8: LG 프리미어 체크박스 및 할인 표시 테스트
**날짜**: 2026-01-18  
**테스트 항목**: LG 프리미어 체크박스 자동 체크 및 할인 표시 확인
**결과**:
- LG 프리미어 약정 적용 체크박스 표시: ✅ 정상 작동 (85군 이상에서만 표시)
- 85군 이상 요금제 선택 시 자동 체크: ✅ 정상 작동
- 85군 미만 요금제 선택 시 자동 해제: ✅ 정상 작동
- LG 프리미어 할인 표시: ✅ 정상 작동 (체크박스 체크 시 -5,250원 표시)

**수정 내용**:
- `selectedPlanGroup` 파싱 로직 개선 (괄호 안의 숫자 추출)
- 85군 이상 요금제 선택 시 `lgPremier` 자동 체크
- 85군 미만 요금제 선택 시 `lgPremier` 자동 해제
- 할인 표시 조건 개선 (요금제군 기준으로만 판단)

### ✅ 테스트 9: 부가서비스 로딩 타이밍 개선 확인
**날짜**: 2026-01-18  
**테스트 항목**: 부가서비스 로딩 완료 후 대리점추가지원금 표시 확인
**결과**:
- 부가서비스 로딩 완료 후 대리점추가지원금 표시: ✅ 정상 작동
- 로딩 순서: 부가서비스 정보 로드 → 대리점추가지원금 표시 ✅

**수정 내용**:
- `calculateInitialPrice` 실행 조건에 `availableAddons`나 `availableInsurances` 로드 여부 확인 추가
- 부가서비스 목록이 로드된 후에만 대리점추가지원금 계산하도록 수정

### ✅ 테스트 10: 요금제군 불일치 테스트
**날짜**: 2026-01-18  
**테스트 항목**: 시세표에서 선택한 요금제군과 개통정보입력페이지의 요금제군 일치 확인
**결과**:
- 요금제군 일치: ✅ 일치함 (시세표와 개통정보입력페이지 요금제군 정확히 일치)
- 부가서비스 초기 선택: ✅ 정확함 (요금제군에 맞는 부가서비스가 정확히 초기 선택됨)

**확인 사항**:
- 다양한 요금제군(33군, 50군, 85군, 115군 등)에서 요금제군 일치 확인
- 각 요금제군에서 부가서비스 초기 선택이 정확한지 확인

### ✅ 테스트 11: 저장 및 조회 테스트
**날짜**: 2026-01-18  
**테스트 항목**: 개통정보 저장 후 구글시트에 저장된 대리점추가지원금 확인
**결과**:
- 저장 시 값: ✅ 실제 계산된 값 저장됨 (전체 대리점추가지원금 220,000원 저장)
- 화면 표시: ✅ 차액만 표시됨 (일반약정 시 51,300원 표시)
- 마진 계산: ✅ 차액금까지 계산해서 제대로 계산됨

**확인 사항**:
- 일반약정 선택 시:
  - 화면 표시: 차액(대리점추가지원금 - 이통사지원금)만 표시 (예: 51,300원)
  - 저장 값: 전체 대리점추가지원금 저장 (예: 220,000원) ✅ 정상
  - 마진 계산: 전체 대리점추가지원금 기준으로 계산 ✅ 정상
- 선택약정 선택 시:
  - 화면 표시: 전체 대리점추가지원금 표시
  - 저장 값: 전체 대리점추가지원금 저장

**설명**:
- 일반약정에서는 고객에게 차액만 보여주지만, 실제 저장할 때는 전체 대리점추가지원금을 저장해야 함
- 마진 계산도 전체 대리점추가지원금 기준으로 계산되어야 함
- 이는 의도된 동작이며 정상 작동함
...
**결과**:
- 전체 흐름: ✅ 정상 작동
- 시세표 → 개통정보입력페이지 → 저장 → 판매일보 조회: ✅ 정상 작동
- 다양한 시나리오 테스트: ✅ 정상 작동

**확인 사항**:
- 시세표에서 모델 선택 후 개통정보입력페이지로 이동
- 개통정보 입력 및 저장
- 판매일보에서 저장된 정보 확인
- 구매대기목록에서 저장된 정보 확인
- 다양한 요금제군, 가입유형, 약정유형 조합 테스트
- 부가서비스 선택/해제 후 저장 테스트

---

## 🎉 테스트 완료 요약

**테스트 완료 날짜**: 2026-01-18  
**총 테스트 항목**: 12개  
**완료된 테스트**: 12개 ✅  
**실패한 테스트**: 0개
...
### 완료된 테스트 목록
1. ✅ 데이터 재빌드 기본 확인
2. ✅ 재빌드 후 정책마진 및 헤더 확인
3. ✅ 개통정보입력페이지 기본 동작 확인
4. ✅ 부가서비스 선택/삭제 테스트
5. ✅ 가입유형 변경 후 부가서비스 제거 문제 해결 확인
6. ✅ 가입유형 변경 시 재계산 확인
7. ✅ 약정유형별 표기 테스트
8. ✅ LG 프리미어 체크박스 및 할인 표시 테스트
9. ✅ 부가서비스 로딩 타이밍 개선 확인
10. ✅ 요금제군 불일치 테스트
11. ✅ 저장 및 조회 테스트
12. ✅ 통합 시나리오 테스트

### 주요 수정 사항 요약
1. **성능 개선**: 데이터 재빌드 속도 최적화 (병렬 처리)
2. **헤더 중복 문제**: 완전 해결
3. **정책마진 0원 문제**: 캐시 무효화로 해결
4. **가입유형 변경 후 부가서비스 제거 문제**: initialSelectedItemsRef 재설정 로직 개선
5. **시세표 이통사지원금 0원 표기 문제**: 시트 데이터 매핑 방식으로 해결
6. **LG 프리미어 체크박스 자동 체크**: 85군 이상 요금제 선택 시 자동 체크
7. **부가서비스 로딩 타이밍 개선**: 부가서비스 로딩 완료 후 대리점추가지원금 표시
...
- [x] **시세표에서 개통정보입력페이지 이동**
  - [x] 시세표에서 행 클릭 시 개통정보입력페이지로 정상 이동
  - [x] 요금제군이 시세표와 일치하는지 확인
  - [x] 초기 대리점추가지원금이 정확히 표기되는지 확인
  - [x] 부가서비스 및 보험 적용시 금액 변경 섹션 정상 표기 확인
  - [x] 부가서비스 로드 후 중복 계산 없음 확인

#### 3. 부가서비스 선택/삭제 테스트
- [x] **부가서비스 선택 변경**
  - [x] 부가서비스 선택 시 대리점추가지원금이 정확히 증가하는지 확인 - 40,000원 정확히 증가
  - [x] 부가서비스 삭제 시 대리점추가지원금이 정확히 감소하는지 확인 - 40,000원 정확히 감소
  - [x] 차액 계산이 `incentive + |deduction|` 공식대로 작동하는지 확인 - 정확히 작동 (30,000 + 10,000 = 40,000)

#### 4. 가입유형 변경 테스트
- [x] **가입유형 변경 후 부가서비스 제거 문제 해결 확인** - ✅ 완료
  - [x] 가입유형 변경 후 첫 번째 부가서비스 제거 시 금액 감소 확인 - ✅ 정상 작동
- [x] **가입유형 변경 (010신규 ↔ MNP ↔ 기변) - 재계산 확인** - ✅ 완료
  - [x] 가입유형 변경 시 대리점추가지원금이 누적되지 않고 재계산되는지 확인 - ✅ 재계산됨
  - [x] 가입유형 변경 시 부가서비스 선택이 초기화되지 않는지 확인 - ✅ 유지됨

### 🟡 우선순위 2: 세부 기능 테스트

#### 5. 약정유형별 표기 테스트
- [x] **일반약정 선택** - ✅ 완료
  - [x] 대리점추가지원금이 차액만큼만 표기되는지 확인 - ✅ 차액 표기됨
  - [ ] 저장 시 실제 계산된 값이 저장되는지 확인 - ⏳ 확인 예정
- [x] **선택약정 선택** - ✅ 완료
  - [x] 대리점추가지원금이 전체 금액으로 표기되는지 확인 - ✅ 전체 금액 표기됨
  - [ ] 저장 시 실제 계산된 값이 저장되는지 확인 - ⏳ 확인 예정

#### 6. LG 프리미어 체크박스 테스트
- [x] **85군 이상 요금제 선택** - ✅ 완료
  - [x] LG 프리미어 체크박스가 자동으로 체크되는지 확인 - ✅ 정상 작동
  - [x] 85군 미만 요금제에서는 체크박스가 해제되는지 확인 - ✅ 정상 작동
  - [x] LG 프리미어 할인 표시 확인 - ✅ 정상 작동

#### 7. 요금제군 불일치 테스트
- [x] **다양한 요금제군 테스트** - ✅ 완료
  - [x] 시세표에서 선택한 요금제군과 개통정보입력페이지의 요금제군이 일치하는지 확인 - ✅ 일치함
  - [x] 부가서비스 초기 선택이 정확한지 확인 - ✅ 정확함
...
## 추가 수정: 가입유형 변경 후 부가서비스 제거 시 금액 감소 문제

### 문제 상황
- 초기 상태에서는 부가서비스 제거/추가가 정상 작동
- 가입유형을 변경(예: 번호이동 → 기기변경) 후 첫 번째 부가서비스 제거 시 금액이 감소하지 않음
- 다시 다른 가입유형으로 변경해도 동일한 문제 발생
...
가입유형 변경 (예: 번호이동 → 기기변경)
  ↓
previousOpeningTypeRef 감지 (584-597번 라인)
  ↓
initialSelectedItemsRef.current = null (리셋)
isInitialLoadRef.current = true
openingTypeChangedRef.current = true
  ↓
calculateInitialPrice 재실행 (457-535번 라인)
  ↓ (formData.openingType 의존성)
storeSupportWithAddon 업데이트
  ↓
❌ 문제: loadAvailableItems는 재실행되지 않음
  ↓
selectedItems는 그대로 유지됨
initialSelectedItemsRef는 null 상태로 남음
```

#### 2. 핵심 문제점

**문제 1: `loadAvailableItems`가 가입유형 변경 시 재실행되지 않음**
- `loadAvailableItems`의 의존성 배열에 `formData.openingType`이 없음 (428번 라인)
- 가입유형 변경 시 `selectedItems`가 재설정되지 않음
- 결과: 이전 가입유형의 `selectedItems`가 그대로 유지됨

**문제 2: `initialSelectedItemsRef` 재설정 타이밍 문제**
- 가입유형 변경 시 `initialSelectedItemsRef.current = null`로 리셋
- 하지만 `selectedItems`가 변경되지 않으면 `initialSelectedItemsRef` 재설정 로직이 실행되지 않음
- `calculateDynamicStoreSupport`에서 `initialSelectedItemsRef.current === null`이면 계산하지 않고 기본값 반환
- 결과: 첫 번째 부가서비스 제거 시 계산이 안 됨

**문제 3: `initialSelectedItemsRef` 재설정 조건 문제**
- 현재 조건: `(isInitialLoadRef.current || initialSelectedItemsRef.current === null) && selectedItems.length >= 0`
- 문제: `selectedItems`가 변경되지 않으면 재설정되지 않음
- 가입유형 변경 후 `selectedItems`가 그대로 유지되면 `initialSelectedItemsRef`가 null 상태로 남음

### 해결 방법

#### 1. `loadAvailableItems` 의존성에 `formData.openingType` 추가
```javascript
// 수정 전 (428번 라인)
...
```
- 가입유형 변경 시 `loadAvailableItems`가 재실행되어 `selectedItems`가 새로운 가입유형에 맞게 재설정됨

#### 2. `initialSelectedItemsRef` 재설정 로직 개선
```javascript
// 수정 전 (553-571번 라인)
...
if ((isInitialLoadRef.current || initialSelectedItemsRef.current === null) && selectedItems.length >= 0) {
    ...
    // selectedItems가 실제로 변경되었을 때만 설정 (무한 루프 방지)
...
```
- `selectedItems`가 실제로 변경되었을 때만 재설정하여 무한 루프 방지
- 가입유형 변경 후 `loadAvailableItems`가 재실행되어 `selectedItems`가 변경되면 자동으로 재설정됨

#### 3. 가입유형 변경 감지 로직에 디버그 로그 추가
```javascript
// 수정 후 (584-597번 라인)
useEffect(() => {
...
### 수정 위치
- `src/components/direct/OpeningInfoPage.js` 428번 라인: `loadAvailableItems` 의존성에 `formData.openingType` 추가
- `src/components/direct/OpeningInfoPage.js` 553-571번 라인: `initialSelectedItemsRef` 재설정 로직 개선
- `src/components/direct/OpeningInfoPage.js` 584-597번 라인: 가입유형 변경 감지 로직에 디버그 로그 추가

### 테스트 시나리오
1. 초기 상태에서 부가서비스 제거/추가 확인
2. 가입유형을 "기기변경"으로 변경 후 첫 번째 부가서비스 제거 시 금액 감소 확인
3. 다시 "번호이동"으로 변경 후 첫 번째 부가서비스 제거 시 금액 감소 확인
4. 가입유형 변경 시 `selectedItems`가 재설정되는지 확인 (콘솔 로그 확인)

### 예상 결과
- 가입유형 변경 시 `loadAvailableItems`가 재실행되어 `selectedItems`가 재설정됨
- `initialSelectedItemsRef`가 새로운 `selectedItems`에 맞게 재설정됨
- 첫 번째 부가서비스 제거 시 금액이 정상적으로 감소함

### ✅ 테스트 결과
**날짜**: 2026-01-18  
**결과**: ✅ 모든 테스트 시나리오 통과
- 초기 상태 부가서비스 제거/추가: ✅ 정상 작동
- 가입유형 변경 후 첫 번째 부가서비스 제거: ✅ 금액 정상 감소
- 문제 해결 확인: ✅ 완료
...
**테스트 완료**: 모든 테스트 항목 통과 (12/12)

### 주요 수정 사항
- ✅ 성능 개선: 데이터 재빌드 속도 최적화 완료
- ✅ 헤더 중복 문제: 완전 해결 완료
- ✅ 정책마진 0원 문제: 완전 해결 완료
- ✅ 가입유형 변경 후 부가서비스 제거 문제: 수정 완료
- ✅ 시세표 이통사지원금 0원 표기 문제: 수정 완료
- ✅ LG 프리미어 체크박스 자동 체크: 수정 완료
- ✅ 부가서비스 로딩 타이밍 개선: 수정 완료
