...
#### 1-3-1. 옵션 파싱에 editUrl 추가
**파일**: `local-discord-bot/bot.js`
**위치**: 라인 56-80 (옵션 파싱 부분)
...
**⚠️ 중요**: 
- 서버에서 `editUrl` 파라미터를 전송하면 파싱됨
- `editUrl`이 없으면 `sheetUrl`을 사용 (2PACX 형식인 경우 실패 가능)
...
try {
  ...
  if (normalMatch) {
    ...
    if (pacxMatch) {
      // 2PACX 형식은 편집 링크 필요
      if (!editUrl) {
        throw new Error('2PACX 형식의 URL은 편집 링크가 필요합니다.');
      }
      // 편집 링크에서 spreadsheetId 추출
      const editIdMatch = editUrl.match(/\/spreadsheets\/d\/([a-zA-Z0-9-_]{44})/);
      if (editIdMatch) {
        spreadsheetId = editIdMatch[1];
      } else {
        throw new Error('편집 링크에서 spreadsheetId를 추출할 수 없습니다.');
      }
    } else {
      // 방법 3: pubhtml 형식
...
#### 2-1. captureSheetViaDiscordBot 함수에 editUrl 파라미터 추가
...
**⚠️ 중요**: 
- `sheetUrl`은 스크린샷용 URL (pubhtml 형식 가능)
- `editUrl`은 엑셀 파일 생성용 편집 링크 (2PACX 형식인 경우 필수)
- `editUrl`이 없으면 `sheetUrl`을 사용하지만, 2PACX 형식인 경우 실패할 수 있음
...
#### 2-1-1. captureSheetViaDiscordBot 함수에 editUrl 파라미터 추가
**파일**: `server/policyTableRoutes.js`
**위치**: 라인 154 (함수 시그니처)
