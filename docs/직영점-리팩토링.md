...
> 직영점 모드(`DirectStoreMode`) & 직영점 관리 모드(`DirectStoreManagementMode`) 전체 구조와  
> 데이터 리팩토링(마스터 시트/ETL/API/프론트)을 한 곳에 정리한 문서입니다.
...
### 8-0. 진행 상태 요약 (마스터/ETL/API)

- **요금제 마스터 (`직영점_요금제마스터`)**
  - [x] 시트 생성 및 헤더 정리
  - [x] ETL 구현 (`rebuildPlanMaster`) 및 쓰기 로직
  - [x] `POST /api/direct/plans-master/rebuild` (수동 재빌드)
  - [x] 링크설정 저장 시 통신사별 자동 재빌드
  - [x] `GET /api/direct/plans-master` 조회 API (운영 중)
- **단말 마스터 (`직영점_단말마스터`)**
  - [x] 시트 생성 및 헤더 정의 (`HEADERS_MOBILE_MASTER`)
  - [x] `GET /api/direct/mobiles-master` 조회 API
  - [x] ETL 구현 (`rebuildDeviceMaster`) 및 시트 쓰기
- **단말 요금/정책 마스터 (`직영점_단말요금정책`)**
  - [x] 시트 생성 및 헤더 정의 (`HEADERS_MOBILE_PRICING`)
  - [x] `GET /api/direct/mobiles-pricing` 조회 API
  - [x] ETL 구현 (`rebuildPricingMaster`) 및 시트 쓰기
- **ETL 트리거 / 프론트 연동**
  - [x] `POST /api/direct/rebuild-master` (통합 재빌드 엔드포인트)
  - [x] `MobileListTab` / `TodaysMobileTab` / `OpeningInfoPage` 의 마스터 기반 데이터 소스 전환 (Fast Track 완료)
...
- **1) 단말 마스터 조회** ✅ (백엔드 구현 완료, 프론트 연동 완료)
...
- **2) 단말 요금/정책 조회** ✅ (백엔드 구현 완료, 프론트 연동 완료)
...
- **3) 요금제 마스터 조회** ✅ (백엔드 구현 완료, 프론트 연동 완료)
  ...
  - 구현 상태:
    - 백엔드:
      - `GET /api/direct/plans-master?carrier=...` 이미 구현 및 운영 중.
    - 프론트:
      - `OpeningInfoPage` 연동 완료.
...
- 현재 상태: ✅ `debug/link-settings` API 구현 및 운영 적용 완료, ⚠ UI에서 시트와의 수동 싱크 검증은 Fast Track 전략으로 생략 (8-8 참고)
...
- 현재 상태: ✅ `POST /api/direct/rebuild-master` 엔트리 포인트 및 정규화 로직 구현 완료, ⚠ `plansSample/devicesSample/pricingSample` 를 시트와 1:1로 대조하는 수동 검증(2-2)은 Fast Track으로 생략됨 (8-8 참고)
...
- 현재 상태: ✅ 요금제/단말/단말요금정책 마스터 시트에 쓰기(ETL)까지 실행하여 운영에 반영 완료, ⏳ 세부 샘플 비교 및 추가 수동 검증은 운영 중 모니터링으로 대체
...
- 현재 상태: ✅ `plans-master` / `mobiles-master` / `mobiles-pricing` API 및 프론트 연동 완료, ⚠ 문서 상 “SK만 부분 전환 후 KT/LG 확장” 계획 대신 **전 통신사 일괄 전환(Fast Track)** 으로 진행됨 (8-8 참고)
...
- 현재 상태: ⏳ 진행 중 – 화면은 마스터 기반으로 전환되었으나, `calculateMobilePrice` 등 레거시 계산/조회 경로 및 `직영점_오늘의휴대폰` 시트 정리는 아직 남은 과제로 관리 (8-8의 잔여 과제 참조)
...
- 현재 상태: ⏳ 미완료 – 실제 시트 삭제/백업 및 코드 수준의 완전한 참조 제거는 아직 수행 전으로, 안정화 이후 별도 정리 작업 필요 (8-8의 Phase 6단계 항목 참조)
...
### 8-8. Fast Track 실행 결과 및 향후 과제 (2025-12-15 업데이트)

사용자 요청에 따라 **중간 수동 검증 단계를 생략하고 전 통신사 일괄 리팩토링(Fast Track)**을 진행했습니다. 이에 따라 생략되거나 추후 확인이 필요한 사항들을 정리합니다.

#### 1. 생략된 검증 절차 (추후 확인 필요)
- **1-2. 링크설정 UI 싱크 검증**: 백엔드 로직으로 대체하고 수동 비교는 생략함. 링크설정이 올바르지 않으면 마스터 데이터가 비어있을 수 있음.
- **2-2. 정규화 로직 정밀 검증**: `rebuild-master` 결과 JSON을 일일이 시트와 대조하는 단계를 건너뛰고 바로 시트에 씀.
- **8-5. 단계별 부분 전환 테스트**: SK/KT/LG를 순차적으로 하지 않고 한 번에 전환함. 통신사별 특이 케이스(LG 프리미어 등)가 잘 반영되었는지 운영 중 확인 필요.

#### 2. 잔여/정리 과제 (Phase 5-1, 6단계)
- **불필요한 레거시 코드 정리**: `calculateMobilePrice` 등 더 이상 메인 로직에서 쓰지 않는 구버전 계산 함수들이 남아있음. 안정화 후 제거 필요.
- **`직영점_오늘의휴대폰` 시트**: 태그 정보가 `직영점_단말마스터`로 이관되었으므로, 더 이상 이 시트를 참조하지 않도록 완전히 끊어내고 시트 삭제/백업 필요.
- **이미지 업로드 동기화**: 현재는 이미지 업로드 시 `직영점_모델이미지`만 갱신하고 캐시를 날림. `직영점_단말마스터`에도 즉시 반영되도록 트리거를 연결하거나 주기적 재빌드가 필요함.

#### 3. 운영 모니터링 포인트
- **초기 로딩 속도**: 마스터 데이터를 한 번에 가져오는 방식이므로 초기 로딩 속도와 데이터 용량(`mobiles-pricing` 크기) 체크.
- **가격 정확도**: 특히 `OpeningInfoPage`에서 선택한 요금제/개통유형에 따른 공시/대리점 지원금이 기존과 10원 단위까지 일치하는지 확인.
