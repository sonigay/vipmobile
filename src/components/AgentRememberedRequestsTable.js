...
function AgentRememberedRequestsTable({ 
  rememberedRequests, 
  onRemoveRequest, 
  onClearAllRequests, 
  onBulkRequest 
    ...
    <Paper elevation={2} sx={{ p: 2, mb: 2 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        <Typography variant="h6" sx={{ color: '#666', fontWeight: 'bold' }}>
          기억된 요청 목록 (관리자모드) ({rememberedRequests.length}개)
        </Typography>
        ...
        <Table size="small" stickyHeader>
          <TableHead>
            <TableRow>
              ...
              <TableCell sx={{ fontWeight: 'bold', backgroundColor: '#f5f5f5' }}>담당자</TableCell>
              ...
              <TableCell sx={{ fontWeight: 'bold', backgroundColor: '#f5f5f5' }}>요청점</TableCell>
          ...
          <TableBody>
            {rememberedRequests.map((request, index) => (
              <TableRow key={request.id} hover>
                ...
                <TableCell>
                  <Chip 
                    label={request.requestedStore ? request.requestedStore.name : '미지정'} 
                    size="small" 
                    color="warning" 
                    variant="outlined"
                  />
...
export default AgentRememberedRequestsTable;
