...
import { MonthlyAwardTab } from './chart/MonthlyAward';
import StructuralPolicyTab from './chart/StructuralPolicy/StructuralPolicyTab';
...
function IndicatorChartTab() {
  ...
  return (
    <Box>
      ...
      {activeSubTab === 3 && <StructuralPolicyTab />}
    </Box>
  );
}
...
function TotalClosingTab({ detailOptions, csDetailType: propCsDetailType, csDetailCriteria: propCsDetailCriteria, presentationMode = false }) {
  ...
  React.useEffect(() => {
    ...
    if (propCsDetailCriteria !== undefined) {
    ...
    return (
      ...
      >
        ...
        <LinearProgress
          ...
          data-capture-exclude="true"
        />
        <Typography variant="body2" color="text.secondary" data-capture-exclude="true">
          {progress}% 완료
        </Typography>
      </Box>
    );
...
function AgentClosingTab() {
  ...
  const groupAgentNames = (agentList) => {
    ...
    agentList.forEach(agent => {
      // 괄호 제거 (예: "홍기현(직영)" → "홍기현")
      const baseName = agent.replace(/\([^)]*\)/g, '').trim();
      if (!grouped[baseName]) {
        grouped[baseName] = [];
      }
      grouped[baseName].push(agent);
    });
    return grouped;
  };
...
function RechotanchoBondTab({ loggedInStore, presentationMode = false }) {
  ...
  useEffect(() => {
    ...
    if (loading) {
      ...
      // 데이터가 로드되었고 로딩이 완료되면 바로 data-loaded 설정
      // Chart.js 렌더링은 MeetingCaptureManager에서 별도로 확인
