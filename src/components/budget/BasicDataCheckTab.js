...
const BasicDataCheckTab = ({ loggedInStore }) => {
  ...
  const loadAllSettings = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/basic-data-settings`, {
        headers: {
      ...
      console.error('기본데이터 설정 전체 로드 오류:', error);
    }
  };
          ...
          {settings.length === 0 ? (
            <Grid item xs={12}>
              <Typography variant="body1" color="text.secondary" sx={{ textAlign: 'center', mt: 4 }}>
                등록된 기본데이터가 없습니다. 기본데이터시트설정 탭에서 추가해주세요.
              </Typography>
            </Grid>
          ) : (
...
export default BasicDataCheckTab;
