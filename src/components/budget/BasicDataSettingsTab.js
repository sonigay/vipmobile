...
const BasicDataSettingsTab = ({ loggedInStore }) => {
  ...
  const loadSettings = async (yearMonth = null) => {
    try {
      setLoading(true);
      const url = new URL(`${API_BASE_URL}/api/basic-data-settings`);
      if (yearMonth) {
      ...
      if (response.ok) {
        ...
        setError('기본데이터 설정을 불러올 수 없습니다.');
      }
    } catch (error) {
      console.error('기본데이터 설정 로드 오류:', error);
      setError('기본데이터 설정을 불러오는 중 오류가 발생했습니다.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveSettings = async () => {
    try {
      const url = editingSetting
        ? `${API_BASE_URL}/api/basic-data-settings/${editingSetting.id}`
        : `${API_BASE_URL}/api/basic-data-settings`;
      ...
      console.error('기본데이터 설정 저장 오류:', error);
  ...
  const handleDeleteSettings = async (id) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/basic-data-settings/${id}`, {
        method: 'DELETE',
        headers: {
          'x-user-role': loggedInStore?.userRole || '',
          'x-user-id': loggedInStore?.contactId || loggedInStore?.id || ''
    ...
    <Box sx={{ p: 3 }}>
        ...
        <Box>
          <Box sx={{ mb: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              <Typography variant="h6">기본데이터 설정 목록</Typography>
          ...
          {loading ? (
            ...
            <TableContainer component={Paper}>
              <Table>
                <TableHead>
                  <TableRow>
                    <TableCell>기본데이터이름</TableCell>
                    <TableCell>기본데이터링크</TableCell>
      ...
      {canAccess && (
        <Dialog open={settingsModalOpen} onClose={handleCloseSettingsModal} maxWidth="md" fullWidth>
          <DialogTitle>
            {editingSetting ? '기본데이터 설정 수정' : '기본데이터 설정 추가'}
          </DialogTitle>
          <DialogContent>
...
export default BasicDataSettingsTab;
