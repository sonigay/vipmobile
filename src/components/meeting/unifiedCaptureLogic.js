...
const slideSpecificHandlers = {
  ...
  async inventoryChart(slideElement, slide, captureTargetElement) {
    ...
    if (config.needsHeaderComposition) {
      ...
      const methods = [
        () => {
          const fixed = slideElement.querySelector('[style*="position: fixed"]');
          if (fixed && fixed.getBoundingClientRect().top < 200) return fixed;
          return null;
        },
        () => {
          const rect = slideElement.getBoundingClientRect();
          return Array.from(slideElement.querySelectorAll('*')).find(el => {
            const elRect = el.getBoundingClientRect();
            const relativeTop = elRect.top - rect.top;
            return relativeTop >= 0 && relativeTop < 200 && elRect.height > 50;
          });
        },
        () => {
          return slideElement.querySelector('[class*="header"], [class*="Header"], .MuiAppBar-root, .MuiToolbar-root');
        },
        () => {
          return slideElement.querySelector('header, [role="banner"]');
        },
        () => {
          ...
          return allElements.find(el => {
            ...
            return relativeTop >= 0 && relativeTop < 200 && 
                   elRect.height > 50 && 
                   elRect.width > rect.width * 0.5 &&
                   !text.includes('재고장표') && // 중간 컨텐츠 헤더 제외
                   (text.includes('회사') || text.includes('company') || 
                    el.querySelector('img, svg') !== null); // 로고 포함
          });
        }
      ];
    ...
    if (tableBox && tableContainer && actualTable) {
      ...
      if (config.needsBoxResize) {
        ...
        const adjustedWidth = tableScrollWidth + paddingLeft + paddingRight + 20;
        const adjustedHeight = tableScrollHeight + paddingTop + paddingBottom + 20;
