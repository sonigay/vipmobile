  ...
  return (
    <Box sx={{ 
      ...
      <Box sx={{ 
        ...
        <Typography variant="subtitle1" fontWeight="bold" sx={{ mb: 1.5, color: '#fff' }}>기존결합 계산식</Typography>
      ...
      <Box sx={{ p: 2, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 1.5, borderBottom: '1px solid #f0f0f0' }}>
        ...
        <Box sx={{ backgroundColor: '#f9fafb', p: 1.5, borderRadius: 1.5, border: '1px solid #e0e0e0' }}>
          ...
          <Typography variant="h6" sx={{ color: '#d32f2f' }}>{Number(result?.bundleDiscount || 0).toLocaleString()}원</Typography>
        </Box>
      </Box>
