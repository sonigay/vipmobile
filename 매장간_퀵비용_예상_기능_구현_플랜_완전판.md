...
- [x] 매장간 거리별 평균 비용 분석
- [x] 거리와 비용 상관관계 시각화
- [x] 시간대별 추이 분석 (월별/주별 비용 변화)
- [ ] 계절별 패턴 분석

##### 데이터 등록/수정/삭제
- [x] 모든 등록 데이터 신규 작성 기능 (권한 체크)
- [x] 모든 등록 데이터 수정/삭제 권한
- [ ] 수정 이력 추적
...
#### 구현 상태: ✅ 백엔드 완료, ✅ 프론트엔드 2차 시각화 적용 (Heatmap/토글 지원)
...
- [x] 프론트엔드 UI 표시 (QuickServiceManagementMode 2차 버전)
  - [x] 지도 기반 시각화로 시/도별 인기·우수 순위를 표시 (마커/열지도 혼합)
  - [x] 인기(등록 건수)와 우수(속도 점수) 지표를 토글할 수 있는 범례/필터 제공
  - [x] 등록 건수/평균 비용/평균 거리/km당 비용 등 지표별 열지도 토글 지원
  - [x] 거리 구간별 평균 비용·km당 비용 표 제공
...
- [x] 여러 사용자가 동일 구간에 데이터 입력 → 평균 계산 확인
- [x] 서비스 품질 정보 표시 확인 (배차/픽업/도착 속도)
- [x] 이상치 데이터 입력 → 경고 표시 확인
- [x] 관리자모드에서 동일 기능 동작 확인
...
- [x] 퀵서비스 관리 모드 접근 권한 확인
- [x] 퀵서비스 관리 모드에서 통계 확인
- [x] 지역별 인기/우수 업체 순위 확인
- [x] 데이터 품질 관리 기능 확인
- [x] 최근 등록 추이 그래프 (월/주 토글) 및 거리 분석 테이블 확인
- [x] 월/주 토글 시 그래프 및 증감 안내 실시간 반영 확인
- [x] 등록/수정/삭제 후 그래프·요약 카드 실시간 갱신 확인
- [x] 통계/히스토리 조회 후 등록일시 자동 보정 확인

### 시나리오 5: 성능 및 에러 처리 테스트

- [x] 캐싱 동작 확인
- [x] 에러 처리 확인 (네트워크 에러, API 에러 등)
...
- ✅ QuickServiceManagementMode 신규 등록·수정·삭제 워크플로
- ✅ QuickServiceManagementMode 최근 등록 추이 & 거리 분석 시각화
- ✅ QuickServiceManagementMode 대시보드/통계 UI 플레이스홀더 제거 및 실데이터 연동
- ✅ App.js quickServiceManagement 모드 전환/복원 로직 정리
- ✅ updateQuickCost, deleteQuickCost API 고도화 및 신뢰도 반영
- ✅ appUpdateService.js 퀵서비스 관리 모드 연동
...
- 월별 등록 추이 지표가 없어 관리자 모드에서 추세 파악이 어려움
...
11. **등록 데이터 수정/삭제 워크플로 추가 (베타)**
   ...
   - 매장 ID/매장명 토글 가능한 자동완성 검색 도입(한 글자 입력으로 실시간 필터링)
12. **지도 시각화 2차 고도화**
   - `server/index.js`에서 매장 좌표 기반 거리 계산(Haversine) 및 지역별/거리별 집계 데이터 제공
   - `/api/quick-cost/statistics` 응답에 `regionAggregates`, `distanceCostAnalysis` 필드 추가
   - `QuickServiceManagementMode` 지도 섹션을 열지도/마커 혼합 뷰로 확장 (등록 건수·평균 비용·평균 거리·km당 비용 토글)
   - 거리 구간별 평균 비용/평균 km당 비용 표(거리 데이터 커버리지 안내 포함) 추가
13. **시간대별 추이 시각화 및 통계 보강**
   - `server/index.js`에서 등록일시 기반 월별 집계를 생성하여 `timeTrends` 응답 필드 제공
   - 월·주별 평균 비용, 등록 건수, km당 비용, 거리 커버리지 등을 계산하고 캐시에 포함
   - `QuickServiceManagementMode`에 최근 등록 추이 섹션 추가, 막대/선 복합 차트로 월/주 추세 표시
   - 월/주 토글을 통한 비교 및 최신 기간 대비 증감 안내(등록 건수·평균 비용) 제공
14. **등록일시 자동 백필 로직 추가**
   - 통계/이력 조회 시 `등록일시`가 비어 있으면 최근 값 또는 현재 시각으로 보정
   - 보정된 시간 정보를 시트에 즉시 반영하여 재호출 시 안정적으로 사용
   - 히스토리/통계 API의 `timeTrends` 및 정렬 로직이 빈 값으로 인해 누락되지 않도록 개선
15. **이상치 탐지 로직 개선 (노선/업체 기반)**
   - 출발/도착 매장쌍, 업체+노선 조합 등 다단계 그룹으로 비용 분포를 계산
   - 그룹별 3건 이상일 때 평균·표준편차·편차율(2σ 초과 또는 30% 이상)을 기준으로 이상치 집계
   - 이상치 정보에 노선 라벨을 포함하고 회사 신뢰도 점수에 반영
16. **업체명 정규화 진행률 산식 수정**
   - 동일 노선 내 변형 업체명(예: 투투퀵 vs 투투퀵서비스)을 비교해 대표명 외 등록 건수를 “정규화 필요”로 집계
   - 총 입력 건수에서 정규화 필요 건수를 제외한 비율을 진행률로 표시
   - 병합 제안 항목에 대표명과 후보명을 함께 노출해 바로 수정할 수 있도록 지원
...
- `src/components/QuickServiceManagementMode.js`
  ...
  - 업체별 통계 개요/신뢰도 상위·등록 상위/저비용 표 추가
  - 매장 ID/매장명 토글형 자동완성 검색을 통한 수정/삭제 워크플로 개선
  - 지도 2차 고도화: 지표별 토글(등록 건수/평균 비용/평균 거리/km당 비용) 및 열지도 스타일 반영, 거리대별 비용 표 추가
- `src/api.js`
  - 통계/품질 API 강제 새로고침 옵션(forceRefresh) 지원
  - `updateQuickCost`·`deleteQuickCost` 함수 추가, history 다중 파라미터 지원
  - `/api/stores` 연동용 `getStores` 함수 추가
- `server/index.js`
  - 매장 좌표를 활용한 거리 계산 유틸리티 및 지역별/거리별 집계 로직 추가
  - `regionAggregates`, `distanceCostAnalysis` 응답 필드 확장
...
- [ ] 지도 시각화 토글별(인기/우수/등록 건수/평균 비용/평균 거리/km당 비용) 마커·열지도 출력 확인
- [ ] 거리대별 비용 분석 테이블 값이 지도 열지도와 일관되게 갱신되는지 확인
