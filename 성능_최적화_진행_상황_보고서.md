...
- **해결 방법**:
  ...
  3. `MobileListRow`에서 `getDisplayValue` 호출 시 `selectedOpeningType` 전달
...
- **수정 파일**:
  ...
  - `src/components/direct/MobileListRow.js`: `getDisplayValue` 호출 시 `selectedOpeningType` 전달

#### 4. 상품 이미지 업로드 문제
- **상태**: 🔍 디버그 로그 추가 완료, 개선 진행 중
- **증상**: 
  - 이미지 업로드 후 Discord에 이미지가 업로드되지 않음
  - 앱 화면에도 상품 이미지가 로드되지 않음
- **원인 분석 중**:
  - Discord 업로드 실패 가능성
  - Google Sheets 저장 실패 가능성
  - 클라이언트에서 이미지 URL 수신 후 UI 반영 실패 가능성
  - 캐시 무효화 타이밍 문제 가능성
- **개선 사항**:
