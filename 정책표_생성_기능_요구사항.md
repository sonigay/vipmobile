...
### CORS 오류 해결 (2024-01-XX) ✅ **해결 완료**
**문제**: 정책표 관련 API 호출 시 CORS 오류 발생
- 오류 메시지: `Request header field x-user-role is not allowed by Access-Control-Allow-Headers in preflight response`
- 영향받는 API 경로 (모든 HTTP 메서드):
  - `/api/policy-tables/tabs` (GET)
  - `/api/policy-table-settings` (GET, POST, PUT, DELETE)
  - `/api/policy-table/user-groups` (GET, POST, PUT, DELETE)
  - `/api/policy-table/generate` (POST)
  - `/api/policy-tables` (GET)
  - `/api/policy-tables/:id` (GET, DELETE)
  - `/api/policy-tables/:id/register` (POST)
  - `/api/policy-tables/:id/refresh-image` (POST)
  - 기타 모든 정책표 관련 API

**참고**: 
- 모든 HTTP 메서드(GET, POST, PUT, DELETE)는 preflight OPTIONS 요청을 먼저 보냅니다
- POST 요청의 "정책표 설정 저장 오류"도 동일한 CORS 원인입니다
- `x-user-id`와 `x-user-role` 헤더를 사용하는 모든 요청이 영향을 받습니다

**원인**:
- `server/index.js`의 `cors` 미들웨어 `allowedHeaders`에 `x-user-id`와 `x-user-role`이 포함되지 않음
- `cors` 미들웨어가 전역 OPTIONS 핸들러보다 먼저 실행되어 preflight 요청을 처리하므로, `cors` 미들웨어의 `allowedHeaders`에 헤더를 추가해야 함

**해결 방법** (✅ 적용 완료):
1. ✅ `server/index.js`의 `cors` 미들웨어 `allowedHeaders`에 `'x-user-id'`, `'x-user-role'` 추가 (145번째 줄)
2. ✅ `server/policyTableRoutes.js`의 라우터 내부 OPTIONS 미들웨어 제거 (전역 핸들러가 처리)
3. ✅ 전역 `app.options('*', ...)` 핸들러에도 `x-user-id`, `x-user-role` 포함 확인 (175번째 줄)
4. ✅ `policyTableRoutes.js`의 `setCORSHeaders` 함수에도 `x-user-id`, `x-user-role` 포함 확인 (568번째 줄)

**⚠️ 중요**: 서버 재시작 필수
- 코드 수정 후 반드시 백엔드 서버를 재시작해야 변경사항이 적용됩니다.
- 서버 재시작 없이는 CORS 오류가 계속 발생합니다.

**관련 파일**:
- `server/index.js` (145번째 줄): `allowedHeaders` 배열에 `'x-user-id'`, `'x-user-role'` 추가 ✅
- `server/index.js` (175번째 줄): 전역 OPTIONS 핸들러 `Access-Control-Allow-Headers`에 `'x-user-id'`, `'x-user-role'` 포함 ✅
- `server/policyTableRoutes.js` (568번째 줄): `setCORSHeaders` 함수에 `'x-user-id'`, `'x-user-role'` 포함 ✅
- `server/policyTableRoutes.js`: 라우터 내부 OPTIONS 미들웨어 제거 ✅
...
- **정책표 추가 모달**: 버튼 클릭 시 모달 표시
  ...
  - **정책표설명** (입력필드, 선택사항): 정책표에 대한 설명을 입력
  - **정책표링크 (편집 링크)** (입력필드): 정책표 구글시트 편집 링크를 입력
    - 시트 ID만 입력해도 자동으로 전체 URL로 변환됨 (예: `1Vy8Qhce3B6_41TxRfVUs883ioLxiGTUjkbD_nKebgrs`)
    - 전체 URL을 입력해도 표준 형식으로 정규화됨 (예: `https://docs.google.com/spreadsheets/d/1Vy8Qhce3B6_41TxRfVUs883ioLxiGTUjkbD_nKebgrs/edit?gid=916754721#gid=916754721`)
    - 정책표생성 탭에서 "구글시트 바로가기" 클릭 시 편집 가능한 링크로 열림
  - **정책표공개링크 (캡처용)** (입력필드, 선택사항): 웹에 게시된 공개 링크(/pubhtml)
    - 예: `https://docs.google.com/spreadsheets/d/e/2PACX-1vQsNcuvP3dm9qtLBBQCTK-NbN9C88sf02b9CAGkVTi9gw8lBgi5Wdn1S6fidCLNlkxp68Fh6DWWqaY3/pubhtml`
    - Puppeteer로 정책표 이미지 캡처 시 사용 (로그인 불필요)
    - 입력하지 않으면 편집 링크를 사용하지만, 공개 링크 사용 권장 (로그인 불필요)
...
1. **구글 시트 URL 접근**
   ```javascript
   // 정책표공개링크가 있으면 사용, 없으면 정책표링크(편집 링크) 사용
   const captureUrl = policyTablePublicLink || policyTableLink;
   await driver.get(captureUrl); // Puppeteer로 접근
   ```
   - **중요**: 공개 링크(`/pubhtml`) 사용 권장 (로그인 불필요)
   - 공개 링크가 없으면 편집 링크를 사용하지만, 로그인이 필요할 수 있음
...
- **정책표이름**: 문자열
- **정책표설명**: 문자열 (선택사항)
- **정책표링크 (편집 링크)**: 구글 시트 편집 URL (문자열)
  - 시트 ID만 입력해도 자동으로 전체 URL로 변환됨
  - 전체 URL을 입력해도 표준 형식으로 정규화됨
  - 사용자가 클릭하여 편집할 수 있는 링크
- **정책표공개링크 (캡처용)**: 구글 시트 공개 URL (문자열, 선택사항)
  - 웹에 게시된 공개 링크 형식 (`/pubhtml`)
  - Puppeteer로 정책표 이미지 캡처 시 사용 (로그인 불필요)
  - 입력하지 않으면 편집 링크를 사용
  ...
  [
    {
      ...
      "policyTableDescription": "기본 정책 변동사항을 관리하는 정책표",
      "policyTableLink": "https://docs.google.com/spreadsheets/d/1Vy8Qhce3B6_41TxRfVUs883ioLxiGTUjkbD_nKebgrs/edit",
      "policyTablePublicLink": "https://docs.google.com/spreadsheets/d/e/2PACX-1vQsNcuvP3dm9qtLBBQCTK-NbN9C88sf02b9CAGkVTi9gw8lBgi5Wdn1S6fidCLNlkxp68Fh6DWWqaY3/pubhtml",
...
- **요청 본문**:
  ```json
  {
    "policyTableName": "기본정책변동",
    "policyTableDescription": "기본 정책 변동사항을 관리하는 정책표",
    "policyTableLink": "1Vy8Qhce3B6_41TxRfVUs883ioLxiGTUjkbD_nKebgrs",
    "policyTablePublicLink": "https://docs.google.com/spreadsheets/d/e/2PACX-1vQsNcuvP3dm9qtLBBQCTK-NbN9C88sf02b9CAGkVTi9gw8lBgi5Wdn1S6fidCLNlkxp68Fh6DWWqaY3/pubhtml",
    "discordChannelId": "1234567890123456789",
    "creatorPermissions": ["AA", "BB", "CC"]
  }
...
- **컬럼 헤더**:
  ```
  ['정책표ID', '정책표이름', '정책표설명', '정책표링크', '정책표공개링크', '디스코드채널ID', '생성자적용권한', '등록일시', '등록자']
  ```
  - **정책표ID**: 고유 식별자 (자동 생성)
  - **정책표이름**: 정책표의 이름
  - **정책표설명**: 정책표에 대한 설명 (선택사항)
  - **정책표링크**: 구글 시트 편집 URL
    - 시트 ID만 입력해도 자동으로 전체 URL로 변환됨
    - 전체 URL을 입력해도 표준 형식으로 정규화됨
    - 사용자가 클릭하여 편집할 수 있는 링크
  - **정책표공개링크**: 구글 시트 공개 URL (선택사항)
    - 웹에 게시된 공개 링크 형식 (`/pubhtml`)
    - Puppeteer로 정책표 이미지 캡처 시 사용 (로그인 불필요)
    - 입력하지 않으면 편집 링크를 사용
  - **디스코드채널ID**: 디스코드 채널 ID
...
- [x] **UI 구성**
  ...
  - [x] 모달 필드:
    - [x] 정책표이름 (TextField)
    - [x] 정책표링크 (편집 링크) (TextField, 시트 ID 또는 전체 URL 입력 가능, 자동 정규화)
    - [x] 정책표공개링크 (캡처용) (TextField, 선택사항, `/pubhtml` 형식)
...
- [x] **백엔드 구현**
  ...
  - [x] 백그라운드 작업 함수:
    - [x] 단계 1: 구글 시트 접근
      - [x] Puppeteer 브라우저 실행
      - [x] 정책표공개링크 우선 사용, 없으면 정책표링크(편집 링크) 사용
      - [x] Puppeteer로 구글 시트 접근
...
#### 1. 정책표생성설정 데이터 검증
- [ ] 정책표이름: 필수, 최대 100자
- [ ] 정책표링크 (편집 링크): 필수, 시트 ID 또는 전체 URL 형식
  - 시트 ID만 입력해도 자동으로 전체 URL로 변환됨
  - 전체 URL을 입력해도 표준 형식으로 정규화됨
- [ ] 정책표공개링크 (캡처용): 선택사항, `/pubhtml` 형식의 공개 링크
  - 입력하지 않으면 편집 링크를 사용하지만, 공개 링크 사용 권장
- [ ] 디스코드채널ID: 필수, 숫자 문자열
...
- **레이아웃**: 
  ...
  - 테이블 컬럼: 정책표이름, 정책표설명, 정책표링크, 정책표공개링크, 디스코드채널ID, 생성자적용권한, 등록일시, 수정/삭제 버튼
...
- [ ] **정책표링크 정규화**: 시트 ID만 입력해도 전체 URL로 자동 변환 (`normalizeGoogleSheetEditLink` 함수 사용)
- [ ] **공개 링크 우선 사용**: Puppeteer 캡처 시 공개 링크(`/pubhtml`) 우선 사용, 없으면 편집 링크 사용
- [ ] **링크 형식 처리**: 
  - 편집 링크: `https://docs.google.com/spreadsheets/d/{SHEET_ID}/edit` 또는 `https://docs.google.com/spreadsheets/d/{SHEET_ID}/edit?gid={GID}#gid={GID}`
  - 공개 링크: `https://docs.google.com/spreadsheets/d/e/{PUBLISH_ID}/pubhtml` (웹에 게시된 링크)
